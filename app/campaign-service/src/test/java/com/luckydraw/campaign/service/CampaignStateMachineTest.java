...
class CampaignStateMachineTest {
    ...
    private Campaign draftCampaign() {
        ...
        return new Campaign("測試活動", now, now.plusDays(7), 10);
    }

    @Test
    @DisplayName("新活動初始狀態為 DRAFT")
    void newCampaign_isDraft() {
        assertThat(draftCampaign().getStatus()).isEqualTo(Campaign.Status.DRAFT);
    }

    @Test
    @DisplayName("DRAFT → ACTIVE 合法")
    void draftToActive_allowed() {
        Campaign c = draftCampaign();
        c.activate();
        assertThat(c.getStatus()).isEqualTo(Campaign.Status.ACTIVE);
    }
    ...
    void activeToEnded_allowed() {
        ...
        c.activate();
        c.end();
        assertThat(c.getStatus()).isEqualTo(Campaign.Status.ENDED);
    }
    ...
    void activeToDraft_rejected() {
        ...
        c.activate();
        assertThatThrownBy(() -> {
            if (c.getStatus() != Campaign.Status.DRAFT) {
