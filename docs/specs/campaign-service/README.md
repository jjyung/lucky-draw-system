...
| **服務範圍** | campaign-service（活動與獎品管理、權重抽獎、冪等控制、事件發布） |
...
| [ADR-004](../../adr/004-weighted-draw-algorithm.md) | 權重隨機抽獎演算法（銘謝惠顧建模、機率總和 100%） |
| [ADR-005](../../adr/005-anti-double-draw-idempotency.md) | 防重複抽獎與冪等（複合冪等鍵、replay 語意） |
| [ADR-006](../../adr/006-anti-overselling.md) | 防超抽（庫存不足降級銘謝惠顧、兩段式扣減） |
| [ADR-007](../../adr/007-async-kafka-spring-cloud-stream.md) | 異步消息（`inventory-commit` 事件） |
| [draw-flow.md](../../architecture/draw-flow.md) | 抽獎生命週期與失敗路徑（本文件行為描述的執行細節來源） |
| [risk-control.md](../../architecture/risk-control.md) | Redis key schema、Lua、冪等判定矩陣、最終一致性 |

> **層級界線**：本文件只定義**業務行為與語意**（use case、business rule、business state、acceptance intent、business data dictionary）。API 路由/參數、OpenAPI schema、DB schema/型別、Redis key 實作、事件 payload 結構屬 **SD 層**，不在本文件範圍；本文件引用之 ADR 中的技術細節僅作為業務語意之佐證，不在此重複設計。
...
campaign-service 是此能力的承載者：它回答「**系統要提供什麼抽獎行為**」，並以業務語意約束「結果如何才算符合目的」。庫存的真相來源（實際出貨扣減）由 inventory-service 承擔，campaign-service 只透過預扣與事件與其協作。
...
- **Main flow:**
  ...
  5. 驗證失敗 → 系統回傳錯誤（`400`/`422`），**整筆配置不落庫**；驗證成功 → 落庫，於後續抽獎生效。
- **Business rule:**
...
- **Acceptance intent:**
  - 總和 ≠ 100% 或機率越界或缺銘謝惠顧 → 拒絕且不落庫，錯誤可理解。
  - 合法配置（總和 = 100%、含銘謝惠顧）成功落庫。
...
- **Main flow:**
  ...
  6. 若命中獎品 → 庫存預扣；庫存不足 → **降級為銘謝惠顧（不重抽）**。
  7. 系統落一筆 `draw_record`（含結果快照與冪等鍵）；計入抽獎次數 1 次。
  8. 若中獎品 → 發布 `inventory-commit` 事件；回傳抽獎結果給 USER。
- **Business rule:**
...
- **Acceptance intent:**
  - 合法單次抽獎回傳一個中獎結果或銘謝惠顧，且落一筆記錄、計次 1 次。
...
- **Main flow:**
  ...
  4. 系統對整批執行 N 次獨立權重抽選；每次抽選命中獎品時**逐筆各自**預扣庫存，庫存不足者**逐筆降級銘謝惠顧**。
  5. 系統落 **N 筆 `draw_record`**；整批**一次性扣除 N 次**抽獎次數。
  6. 對每筆中獎品的記錄發布 `inventory-commit`；回傳 N 筆結果。
- **Business rule:**
  - 批次抽獎 = 「單一請求、伺服端 N 次獨立抽選」，整批由**單一 Idempotency-Key** 保護。
  - 整批副作用（N 筆落庫、N 次計次、N 筆事件）**僅執行一次**；replay 不重複執行。
  - 抽獎次數**一次扣 N**（僅成功產生結果的請求計次）。
  - 每筆抽選獨立計算機率與庫存，互不影響（一筆庫存不足不影響同批其他筆）。
  - 批次與「並發多個單次請求」（UC-4）為**兩種並存模式**，都需支援。
- **Acceptance intent:**
  - `count = N` 請求回傳 N 筆結果、落 N 筆記錄、計次 N 次。
...
| `WIN` | 中獎（命中任一獎品） | 權重抽選命中獎品且庫存預扣成功 |

> **註**：`VOID`（撤銷）是 inventory-service 在 DB 條件更新失敗時的**補償狀態**（ADR-006 Path C），屬下游補償語意，非抽獎本身之正常結果；campaign-service 的業務結果型別僅上述兩種，`VOID` 由下游回寫，SA 於此僅註記其存在。
...
2. **使用者看到的是本系統狀態還是 upstream 狀態？**
   ...
   - 獎品**實際剩餘庫存**是 inventory-service 的 upstream 狀態；campaign-service 只透過預扣（Redis 加速層）與事件協作，不持有庫存真相。USER 看到的「可抽性」由本系統活動狀態決定，「庫存精確餘額」不即時暴露。
...
4. **哪些欄位不能被使用者直接改？**
   - 中獎結果（`result_type` / `prize_id`）：由伺服端演算法決定，client **不得指定**。
   - 個人抽獎次數（`draw_count`）：伺服端計數器，client 不得增減。
   - 複合冪等鍵中的 `userId` / `campaignId`：由伺服端依 token 與路徑決定，client 僅提供 `idempotencyKey`。
...
### 5.3 draw_record（抽獎記錄）

| 欄位 | 業務意義 | 必填 | 合法值 | 真實來源 | 敏感性 |
|------|----------|------|--------|----------|--------|
| `user_id` | 執行抽獎之使用者 | 是 | 有效使用者識別 | 由 token 決定（Gateway 轉發），非 client 指定 | personal |
| `campaign_id` | 抽獎所屬活動 | 是 | 有效活動識別 | 由請求路徑決定（伺服端不信任 client） | internal |
| `idempotency_key` | 複合冪等鍵中 client 提供之部分（一次點擊一個 UUID） | 是 | 非空 UUID | Client 提供（Gateway 強制存在） | internal |
| `result_type` | 本次抽獎結果型別 | 是 | `WIN` / `THANK_YOU` | campaign-service 抽獎演算法（伺服端決定） | internal |
| `prize_id` | 中獎獎品識別（銘謝惠顧時指向 `THANK_YOU` 獎品） | 條件 | 有效獎品識別 | 抽獎演算法結果 | internal |
| 抽獎次數 (`draw count`) | 使用者於活動期間之**累計**抽獎次數（派生計數，維度 = campaign） | 派生 | 非負整數 ≤ `draw_limit` | campaign-service 計數器；`draw_record` 為稽核真相 | personal |

> **語意註記**：`draw count` 是「使用者 × 活動」維度的派生計數（TTL 對齊活動結束），不是單一 `draw_record` 的欄位；`draw_record` 是該計數的稽核證據（每筆成功抽獎對應 +1，批次 +N）。
...
- THEN 系統回傳驗證錯誤（`400`/`422`），且整筆配置**不落庫**，活動仍為原有效配置
...
- THEN 系統回傳驗證錯誤（因活動將無法產生「未中獎」結果），配置不落庫

### 6.2 單次抽獎 (Single Draw)

**AC-CAMP-004 — 單次抽獎成功**（`FR-CAMP-07`, `FR-CAMP-10`）
- GIVEN 活動為 `ACTIVE`、USER 有剩餘次數、請求帶有效 `Idempotency-Key`
- WHEN USER 提交單次抽獎（`count = 1`）
- THEN 系統以權重隨機選取一個獎品、落一筆 `draw_record`、計次 1 次、回傳一個結果（中獎或銘謝惠顧）
...
- WHEN USER 提交抽獎請求
- THEN 系統回傳 `429 Too Many Requests`，不落 `draw_record`、不扣庫存、不計次
...
- GIVEN 活動 `ACTIVE`、USER 剩餘次數 ≥ N、單一 `Idempotency-Key`
- WHEN USER 提交 `count = N` 批次抽獎
- THEN 系統落 N 筆 `draw_record`、回傳 N 筆結果、每筆命中獎品時**逐筆各自**預扣庫存
...
- WHEN 相同複合鍵重送
- THEN 回傳原 N 筆結果，不重複落庫、不重複扣 N 次、不重複發布事件
...
- THEN 各獎品命中比例收斂至配置機率（誤差在統計容許範圍內），累計區間邊界值（`r` 恰落於界線）行為正確

### 6.8 事件發布 (Event Publication)

**AC-CAMP-016 — 中獎發布 inventory-commit**（`FR-CAMP-17`）
- GIVEN 抽獎結果為中獎（`WIN`）
- WHEN 抽獎完成
- THEN 系統發布 `inventory-commit` 事件（含抽獎記錄識別、獎品識別、數量），且對該記錄僅發布一次
...
| 項目 | 說明 | 後續層 |
|------|------|--------|
| 身份驗證 / 授權實作 | JWT 驗證、RBAC 判定屬 Auth Service + API Gateway（`FR-GW-*`、`FR-AUTH-*`），campaign-service 僅信任轉發 claim | Auth/Gateway spec |
| 庫存真相來源與補償內部邏輯 | `UPDATE ... WHERE stock > 0`、`draw_reversal`/`VOID`、對帳 job 屬 inventory-service（`FR-INV-*`） | inventory-service spec |
...
| Redis key schema / Lua script 實作 | 屬風控實作細節（ADR-003、risk-control.md），本文件只定義業務語意 | SD |
...
| FR-CAMP-13 | 複合冪等鍵（Redis + DB UNIQUE） | UC-4, UC-5 | AC-CAMP-012 |
