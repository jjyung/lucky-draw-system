...
2. 活動瀏覽（列表與詳情，公開查詢）
3. 獎品與機率配置（含銘謝惠顧、總和 100% 驗證）
4. 權重隨機抽獎（單次／批次／並發單次）
5. 個人抽獎次數上限控管（**活動期間總額**）
6. 防重複抽獎（複合冪等鍵 + replay 語意）
7. 通知庫存扣減（抽中獎品時）
...
### UC-6 瀏覽活動 (Browse Campaign — 列表與詳情)

- **Actor:** PUBLIC（訪客，未登入）／USER
- **Precondition:** 無（公開功能，不需登入）。
- **Main flow:**
  1. 訪客請求活動列表，取得可參與活動之摘要（名稱、時間、狀態，不含管理欄位）。
  2. 訪客請求特定活動詳情，取得該活動資訊（名稱、起訖時間、狀態、獎品概要），供抽獎前展示。
  3. 系統回傳對應資料；活動不存在回傳 `404`。
- **Business rule:**
  - 活動列表／詳情為**公開功能**（`FR-GW-06`），不需身份憑證即可存取（免登入）。
  - 列表與詳情**不含管理欄位**（如營運內部備註、庫存精確餘額），只提供 USER 決策參與所需之資訊。
  - 此為 `FR-CAMP-01`「活動 CRUD」之 **R（讀取）** 面；C/U/狀態轉移（UC-1/UC-3）屬 ADMIN。
- **Acceptance intent:**
  - 未登入存取活動列表／詳情成功放行。
  - 活動不存在回傳 `404`。
  - 回傳內容不含管理欄位。
- **Traceability:** `FR-CAMP-01`（R）, `FR-GW-06`, `FR-X-01`

> **註**：本 UC 為覆蓋審計回寫補入——原 SA 未明列「活動瀏覽」，但 SD 已依隱含需求實作 `campaign-campaigns-001/002` 並標為開放點 #1；此處將隱含需求顯式化，消除 FR→UC→API 的斷點。
...
|-------|----------|-----------|------------|
| FR-CAMP-01 | 活動 CRUD + 狀態機（DRAFT/ACTIVE/ENDED） | UC-1, UC-3, UC-6 | AC-CAMP-005 |
